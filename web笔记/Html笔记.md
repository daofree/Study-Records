...
## web概念概述
	...
	* 软件架构：
		1. C/S: Client/Server 客户端/服务器端----------安卓
			...
			* 缺点：
				1. 开发、安装，部署，维护 麻烦
		2. B/S: Browser/Server 浏览器/服务器端-----基于互联网的项目
			* 只需要一个浏览器，用户通过不同的网址(URL)，客户访问不同的服务器端程序
			* 优点：
				1. 开发、安装，部署，维护 简单
			* 缺点：
				1. 如果应用过大，用户的体验可能会受到影响,比如网页游戏
				2. 对硬件要求过高,网速带宽内存
...
注意：6/7要与CSS一起使用
		...
		8. 表格标签：
			...
			* th：定义表头单元格，有样式
			* <caption>：表格标题
			没有样式，增强可读性，用与CSS结合，类似H5的语义化标签
			* <thead>：表示表格的头部分
			* <tbody>：表示表格的体部分
			* <tfoot>：表示表格的脚部分,不在最后，也永远展示在下面

## 案例：旅游网站首页--多行异列---布局方案：用表格嵌套，就不用考虑是否需要考虑合并单元格。最外层永远一列
	1. 确定使用table来完成布局	
	2. 如果某一行只有一个单元格，则使用<tr><td></td></tr>
	3. 如果某一行有多个单元格，则使用 表格嵌套，不用考虑合并单元格
		<tr>
			<td>
				<table></table>
			</td>
